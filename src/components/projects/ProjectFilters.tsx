...
export type ProjectFilter = "Tous" | ProjectCategory;

const filters = [
  { label: "Tous", icon: Grid2X2 },
...
export function ProjectFilters({ activeCategory, onCategoryChange, search, onSearchChange }: ProjectFiltersProps) {
  return (
    <div className="mx-auto grid max-w-[1200px] gap-4 px-5 pb-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-center lg:px-8">
      <div className="flex min-w-0 gap-2 overflow-x-auto pb-1 scrollbar-none" aria-label="Filtrer les projets">
        {filters.map((filter) => (
          <Button
            key={filter.label}
            type="button"
