  ...
  return (
    <article className="group flex min-h-[405px] flex-col overflow-hidden rounded-card border border-border bg-card shadow-card transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:shadow-card-hover">
      ...
      <div className="flex flex-1 flex-col px-4 pb-4 pt-3.5 sm:px-5 sm:pb-5">
        ...
        <h2 className="mt-3 text-[17px] font-extrabold leading-tight text-foreground">{project.name}</h2>
        <p className="mt-2 line-clamp-3 text-[13px] leading-[1.55] text-muted-foreground">{project.description}</p>
        <a
          href={`#${project.slug}`}
        ...
        >
          Voir le projet <ArrowRight className="h-3.5 w-3.5" aria-hidden="true" />
        </a>
      </div>
    </article>
  );
